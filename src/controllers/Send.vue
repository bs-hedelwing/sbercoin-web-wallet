<template>
  <v-card>
    ...
    <v-card-text>
      <v-form>
        <v-text-field :label="$t('send.address')" v-model.trim="address" required></v-text-field>
        <v-layout>
          <v-flex xs9>
            <v-text-field :label="$t('send.amount')" v-model.trim="amount" required></v-text-field>
          </v-flex>
          <v-flex xs3>
        ...
        <v-text-field v-if="symbol !== 'SBER'" :label="$t('send.gas_price')" v-model="gasPrice"></v-text-field>
        <v-text-field v-if="symbol !== 'SBER'" :label="$t('send.gas_limit')" v-model="gasLimit"></v-text-field>
        <v-text-field :label="$t('send.fee')" v-model.trim="fee" required></v-text-field>
      </v-form>
    </v-card-text>
    <v-card-actions>
      <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    initTokens() {
      const tokenList = [{ text: 'SBER', value: 'SBER' }]

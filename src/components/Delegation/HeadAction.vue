  ...
  <section class="delegation-head-action">
    <section class="main">
      ...
      <section>
        ...
        <v-btn round info @click.native="refreshData" color="#1976d2">
        ...
        <v-card-text>
          <v-form v-model="formValidate" ref="addDelegationForm">
            <v-layout wrap>
              ...
              <v-flex xs8 offset-xs2>
                <v-text-field :label="$t('common.info.gas_price')" v-model="info.gasPrice" type="number" min="0" suffix="greph"/>
              </v-flex>
              <v-flex xs8 offset-xs2>
...
export default {
  ...
  methods: {
    async confirmSend() {
      ...
      // 将地址转换为 hex
      const hexAddress = sbercoin.address.fromBase58Check(this.info.stakerAddress).hash.toString('hex')

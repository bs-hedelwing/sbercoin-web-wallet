import sbercoinJsLib from 'sbercoinjs-lib'
import btcApp from '@ledgerhq/hw-app-btc'
import transportU2f from '@ledgerhq/hw-transport-u2f'
import BigNumber from 'bignumber.js'
import SafeBuffer from 'safe-buffer'
import OPS from 'sbercoin-opcodes'
...
export default class Ledger {
    ...
    static async connect() {
        const sbercoin = new btcApp(await transportU2f.create())
        // ensure in Sbercoin App
        const pubkeyRes = await sbercoin.getWalletPublicKey(Ledger.defaultPath)
        if (pubkeyRes.bitcoinAddress[0] !== 'Q') {
            throw 'Not Sbercoin App'
        }
        return new Ledger(sbercoin)
    }

    static async generateTx(keyPair, ledger, path, from, to, amount, fee, utxoList, rawTxFetchFunc = () => {}) {
        const amountSat = new BigNumber(amount).times(1e7)
        const feeSat = new BigNumber(fee).times(1e7)
        const pubkeyRes = await ledger.sbercoin.getWalletPublicKey(path)
        ...
        const inputs = []
        const paths = []
        const selectUtxo = sbercoinJsLib.utils.selectTxs(utxoList, amount, fee)
        const rawTxCache = {}
        for (let i = 0; i < selectUtxo.length; i++) {
            ...
            inputs.push([
                await ledger.sbercoin.splitTransaction(rawTxCache[item.hash]),
                item.pos
            ])
        }
        const outputs = new sbercoinJsLib.TransactionBuilder(keyPair.network)
        ...
        return await ledger.sbercoin.createPaymentTransactionNew(inputs, paths, undefined, outputsScript)
    }
    ...
    static async generateSendToContractTx(keyPair, ledger, path, from, contractAddress, encodedData, gasLimit, gasPrice, fee, utxoList, rawTxFetchFunc = () => {}) {
        ...
        const outputs = new sbercoinJsLib.TransactionBuilder(keyPair.network)
        const contract = sbercoinJsLib.script.compile([
            OPS.OP_4,
